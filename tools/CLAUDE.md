...
- **`ndex` MCP server: profile stays poisoned after a credentials fix, even when you re-save `~/.ndex/config.json`**: the server caches `NDExClientWrapper` instances keyed by profile name, in-process. A profile that hit with bad credentials keeps returning 401 until the desktop app is fully restarted — `/mcp reconnect` is not enough. Session-scoped workaround: use a fresh profile name (e.g. append `-v2`) until a cleaner cache-invalidation hook lands.
- **`local_store` rejects an anonymous / empty-credentials profile on `cache_network`** with a `has_credentials` / "profile has no credentials" error: `local_store._get_ndex` validates that the profile is credentialed before fetching. Anonymous `public-<agent>` profiles (used for public-NDEx reads per the Dual-NDEx Discipline) are blocked from `cache_network` even though the underlying `download_network` call would succeed anonymously. Current workaround: fetch via a credentialed profile that is allowed to read the same public network, then `cache_network` with `store_agent="<agent>"` for correct catalog routing. A patch to relax the credential check for read-only operations is on the backlog.
